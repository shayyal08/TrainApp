...
class SearchTrainViewController: UIViewController {
    ...
    @IBAction func addToFavourite() {
        if(self.sourceTxtField.text == "" || self.destinationTextField.text == "") {
            self.showAlert(title:"" , message: "Enter source and destination fields", actionTitle: "Okay")
        
        } else {
            presenter?.saveFavourites(names:[self.sourceTxtField.text!, self.destinationTextField.text!])
            self.getFavorites()
        }
    }
    
    func getFavorites() {
        favoriteNames = self.presenter?.getFavorites() ?? []
    }

    override func viewWillAppear(_ animated: Bool) {
        if stationsList.count == 0 {
            //SwiftSpinner.useContainerView(view)
            //SwiftSpinner.show("Please wait loading station list ....") //shilpa: commented
...
extension SearchTrainViewController:PresenterToViewProtocol {
    ...
    func showErrorMessage(error:TypeError) {
        trainsListTable.isHidden = true
        hideProgressIndicator(view: self.view)
        showAlert(title: "", message: "Sorry, Please try after some time ", actionTitle: "Okay")
    }
    ...
    func saveFetchedStations(stations: [Station]?) {
        if let _stations = stations {
          self.stationsList = _stations
            print(self.stationsList)
        }
       // SwiftSpinner.hide()//shilpa: Commented
        hideProgressIndicator(view: self.view)

    }
    
    func getdropDownNameList() ->[String] {
        var stationNames = stationsList.map {$0.stationDesc}.sorted()
        if(self.favoriteNames.count > 0) {
           stationNames.insert(contentsOf:
                                self.favoriteNames, at: 0)
        }
        return stationNames
     }
}
...
extension SearchTrainViewController:UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        let stationNames = stationsList.map {$0.stationDesc}.sorted()
        
        print(stationNames)
        dropDown.dataSource = getdropDownNameList()
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if let inputedText = textField.text {
            ...
            dropDown.dataSource =  stationsList.map {$0.stationDesc}.filter{$0.localizedCaseInsensitiveContains(desiredSearchText)}.map{$0.capitalized}//shilpa
            dropDown.show()
